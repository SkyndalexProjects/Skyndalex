...
const equalizerPresets = [
    { id: 'standard', name: 'Standard', gains: [
            { band: 0, gain: 0.0 },
            { band: 1, gain: 0.0 },
            { band: 2, gain: 0.0 },
            { band: 3, gain: 0.0 },
    ...
    { id: 'bass-boost', name: 'Bass Boost', gains: [
            { band: 0, gain: 0.12 },
            { band: 1, gain: 0.12 },
            { band: 2, gain: 0.10 },
            { band: 3, gain: 0.08 },
    ...
    { id: 'treble-boost', name: 'Treble Boost', gains: [
            { band: 0, gain: -0.05 },
            { band: 1, gain: -0.05 },
            { band: 2, gain: -0.04 },
            { band: 3, gain: -0.02 },
...
export default async function channels(fastify: FastifyInstance) {
    fastify.post(
        "/radio_player",
        async (
            request: FastifyRequest<{ Params: { id: string }; Body: { presetId: string; guildId: string } }>,
            reply: FastifyReply,
        ) => {
            const guildId = request.params.id;
            const { presetId } = request.body as { presetId: string; guildId: string };
