...
export async function run(client, interaction) {
    ...
    await client.prisma.economy.upsert({
        ...
        update: {
            wallet: (parseInt(user?.wallet || '0') - interaction.options.getInteger('amount')).toString()
        }
    });
    ...
    const embedAdded = new EmbedBuilder()
        .setColor('#00ff00')
        .setDescription(
            `Removed ${interaction.options.getInteger('amount').toString()} ${settings?.currency || '🌈'} from <@${
                member.id
