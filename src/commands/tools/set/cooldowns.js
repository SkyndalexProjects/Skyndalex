...
export async function run(client, interaction) {
  ...
  const embedSet = new EmbedBuilder()
    .setDescription(`Cooldown for \`${command}\` set to \`${seconds}\` seconds`)
    .setColor('Blurple');
  await interaction.reply({ embeds: [embedSet] });
}
export const data = new SlashCommandSubcommandBuilder().setName("cooldowns").setDescription("Set cooldowns")
.addIntegerOption(option => option.setName("seconds").setDescription("Cooldown in seconds").setRequired(true))
