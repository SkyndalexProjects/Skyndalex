...
export async function run(client, interaction) {
    if (interaction.options.getSubcommand() === 'create') {
        const embed = new EmbedBuilder()
            ...
            .setDescription(`You can create a custom bot by clicking the button below.`)
        ...
        const getBot = await client.users.fetch(findUserBots[0]?.clientId).catch(() => null);
        ...
        if (interaction.options.getSubcommand('bots')) {
            ...
            const embed = new EmbedBuilder().setTitle('Your custom bots').setColor('DarkButNotBlack');

            for (const bot of findUserBots) {
                const getBot = await client.users.fetch(bot.clientId).catch(() => null);

                embed.setFields(`Custom bot: ${getBot?.username || 'Unknown'}`, `ID: \`${bot.id}\``);
            }
...
export const data = new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand => subcommand.setName('create').setDescription('Create a custom bot'))
    .addSubcommand(subcommand => subcommand.setName('delete').setDescription('Delete a custom bot'))
    .addSubcommand(subcommand => subcommand.setName('manage').setDescription('Manage your custom bot'));
