...
import { EmbedBuilder, StringSelectMenuBuilder, ButtonBuilder } from "#builders";
...
export async function run(
    ...
    if (favourties.length <= 0) {
        return interaction.reply({ content: "No favourties found", ephemeral: true })
    };
    ...
    const select = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
        new StringSelectMenuBuilder(client, interaction.locale)
            ...
            .addOptions(favourties
                .slice((currentPage - 1) * radiosPerPage, currentPage * radiosPerPage)
                .map((favourite) => ({
