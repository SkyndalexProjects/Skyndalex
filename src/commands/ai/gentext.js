...
export async function run(client, interaction) {
    ...
    try {
        ...
        if (!result.generated_text)
            return interaction.editReply('An error occurred while generating text (Probably model time out)');
        ...
        console.error(e);
        await interaction.editReply('An error occurred while generating text (Probably model time out)');
    }
}
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option =>
        option.setName('input').setDescription('Input for the AI').setMaxLength(2000).setMinLength(1).setRequired(true)
    );
