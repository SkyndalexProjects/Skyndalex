...
export async function run(
    ...
    try {
        ...
        if (!data) return interaction.reply(client.i18n.t("GAME_SERVER_NOT_FOUND", { lng: interaction.locale }))
        const embed = new EmbedBuilder(client, interaction.locale) 
            .setTitle(String(data.name))
            .setColor("Green")
            .setFields([
                { name: "IP", value: String(data.connect), inline: true },
                { name: "Players", value: `${data.numplayers}/${data.maxplayers}`, inline: true },
                { name: "Map", value: String(data.map) || "None", inline: true },
                { name: "Ping", value: String(data.ping), inline: true },
            ]);
        return interaction.reply({ embeds: [embed] });
    } catch (e) {
        return interaction.reply(client.i18n.t("GAME_SERVER_FAILED", { lng: interaction.locale }))
    }
}
