...
export async function run(client, interaction) {
    ...
    const embed = new EmbedBuilder()
        .setTitle(`${data.name} : \`${data.raw.vanilla.connect}\` | Ping: ${data.raw.vanilla.ping}ms`)
        .addFields(
            {
                name: 'Map',
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('server').setDescription('Server IP').setRequired(true));
