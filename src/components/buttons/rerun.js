...
export async function run(client, interaction) {
    try {
        ...
        if (response?.error) {
            const errorEmbed = new EmbedBuilder().setColor('#e74c3c').setDescription('❌ Error: ' + response.error);
        ...
        const newEmbed = new EmbedBuilder()
            .setDescription(`✅ Generated img "**${prompt}**" requested from input by **${interaction.user.username}**`)
            .setColor('#12ff00');
        ...
        const like = new ButtonBuilder().setCustomId('like').setLabel('👍').setStyle(ButtonStyle.Primary);

        const dislike = new ButtonBuilder().setCustomId('dislike').setLabel('👎').setStyle(ButtonStyle.Primary);
        ...
        const secondMessage = new EmbedBuilder().setColor('#12ff00').setDescription('✅ Your image is ready!');
