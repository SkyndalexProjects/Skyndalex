...
export async function run(client, interaction) {
    ...
    const getBot = await client?.users?.fetch(interaction.values[0]).catch(() => null);
    ...
    const select = new StringSelectMenuBuilder().setCustomId('customBotSelect').setPlaceholder('Choose a custombot!');
