...
export const auth = betterAuth({
    ...
    socialProviders: {
        discord: {
            ...
            getUserInfo: async (tokens) => {
                ...
                return {
                    user: {
                        name: json.username,
                        email: `${json.id}@discord.skyndalex.com`,
                        image: json.avatar
        ...
        huggingface: {
            clientId: process.env.HF_CLIENT_ID as string,
            clientSecret: process.env.HF_APP_SECRET as string,
        }
    },
    user: {
        additionalFields: {
    ...
    account: {
        accountLinking: {
            enabled: true,
            trustedProviders: ["discord", "huggingface"],
    ...
    advanced: {
        ...
        crossSubDomainCookies: {
            enabled: isProd,
            domain: isProd ? (process.env.OAUTH_DOMAIN as string) : undefined,
        },
