...
export async function messageCreate(client, message) {
    ...
    if (message.channel.id === getCurrentSettings[0]?.aiChannel && !message.author.bot) {
    ...
    if (!message.author.bot && message.channel.type === ChannelType.PublicThread) {
        const fetchStarterMessage = await message.channel.fetchStarterMessage();

        if (fetchStarterMessage.embeds[0].data.footer.text.startsWith('Generated by AI')) {
            try {
                const messages = await message.channel.messages.fetch({ limit: 100 });
                const pastUserMessages = messages.filter(m => m.author.id === message.author.id);
                ...
                console.error(e);
                await message.reply('An error occurred while generating text (Probably model time out)');
            }
        }
    }
}
