...
export async function voiceStateUpdate(client, oldState, newState) {
    ...
    if (getSettings?.voiceLogsChannel) {
        ...
        if (oldState.channel !== newState.channel) {
            ...
            const action = newState.serverDeaf ? 'guild-deafened' : 'guild-undeafened';
            description = `**${newState.member.user.username}** was ${action} in \`${newState.channel.name}\``;
        ...
        if (description) {
            ...
            await client.channels.cache.get(getSettings.voiceLogsChannel).send({ embeds: [embed] });
        }
    }
    ...
    if (oldState.channel && (!newState.channel || oldState.channel.id !== newState.channel.id)) {
        if (oldState.channel.members.size === 1) {
            ...
            if (existingPlayer) {
                ...
                if (getSettings?.voiceLogsChannel)
                    await client.channels.cache.get(getSettings?.voiceLogsChannel).send({ embeds: [stoppedSong] });
            }
        }
    }
}
