...
export async function voiceStateUpdate(client, oldState, newState) {
	...
	if (oldState.channel !== newState.channel) {
        ...
        for (const key in availableStateKeys) {
            if (oldState[key] !== newState[key]) {
               embed.setDescription(`User ${newState.member.user.username} was **${newState[key] ? "un" : ""}${availableStateKeys[key]}** in \`${oldState.channel.name}\``);
            }
        }
	}
    ...
    if (description) {
        ...
        client.channels.cache.get("1071407744894128178").send({ embeds: [embed] }); // channel id hardcoded temporary
    }
}
