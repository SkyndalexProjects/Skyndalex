...
export class Loaders {
	static async loadCommands(path) {
		...
		for (const category of dir.filter((file) => !file.endsWith(".js"))) {
			...
			for (const file of files.filter((file) => !file.endsWith(".js"))) {
				const subCommands = await readdir(
					new URL(`${path}/${category}/${file}`, import.meta.url),
				);

				for (const v of subCommands) {
					const command = await import(
						`${path}/${category}/${file}/${v}`
					);
					commands.set(
						`${file.split(".")[0]}/${v.split(".")[0]}`,
						command,
					);
				}
			}
		...
		console.log(
			...
			)}] ${chalk.greenBright("[HANDLERS]")} ${chalk.greenBright(
	...
	static async loadComponents(path) {
		...
		for (const file of files) {
			const component = await readdir(
				new URL(`${path}/${file}`, import.meta.url),
			);
			for (const comp of component) {
				...
				if (!componentFile)
					return console.log(
						`Component ${comp} does not have a run function`,
					);
		...
		console.log(
			`[${chalk.whiteBright(
				...
				chalk.bold(`Loaded ${components.size} components.`),
			)}`,
		);
	...
	static async loadModals(path) {
		...
		for (const file of files) {
			...
			if (!modal)
				return console.log(
					`Modal ${file} does not have a run function`,
				);
		...
		console.log(
			`[${chalk.whiteBright(
				...
				chalk.bold(`Loaded ${modals.size} modals.`),
			)}`,
		);
		return modals;
	}
